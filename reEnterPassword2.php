...
if (isset($_POST['reset'])) {
  ...
  if ($pass === $repass) {
    ...
    $sql = "SELECT * FROM staff WHERE staff_email = '$key' AND md5(staff_password) = '$reset'";

    $result = $conn->query($sql);
    if ($result->num_rows > 0) {

      $sql = "UPDATE staff SET staff_password = '$pass' WHERE staff_email = '$key'";

      if ($conn->query($sql) === TRUE) {
        echo '<script>alert("Success Update. Please login");</script>';
        echo "<script>window.location.assign('login2.php')</script>";
      } else {
        echo '<script>alert("Failed to update");</script>';
      }
    } else {
      $sql = "SELECT * FROM instructor WHERE instructor_email = '$key' AND md5(instructor_password) = '$reset'";

      $result = $conn->query($sql);
      if ($result->num_rows > 0) {

        $sql = "UPDATE instructor SET instructor_password = '$pass' WHERE instructor_email = '$key'";
      ...
      <div>
        <div class="card" style="margin-top: 20px;">
          ...
          <form method="post">
            ...
            <input id="username" type="password" placeholder="Enter password" name="password" required><br>

            <label style="float: left; padding-left:45px"><b>Re-Password</b></label><br>
            <input id="username" type="password" placeholder="Enter re-password" name="repassword" required><br>
